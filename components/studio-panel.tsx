...
export function StudioPanel({ 
  ...
  // This useEffect fetches the outline when a new file is selected
  useEffect(() => {
    if (selectedFile) {
      ...
      const jsonFilename = selectedFile.name.replace(/\.pdf$/i, '.json');
      ...
      // Clear outline when no file is selected
      setOutline([]);
      setOutlineError(null);
    }
  }, [selectedFile]);

  // Handler to jump to a specific page in the PDF
  const handleGoToPage = (pageNumber: number) => {
    if (adobeViewer) {
      ...
      console.warn("Adobe Viewer instance is not ready.");
      alert("The PDF viewer is not ready. Please wait a moment and try again.");
    }
  };

  // This function renders the main content of the panel
  const renderStudioContent = () => {
    // --- THIS IS THE FIX ---
    // Create a new array that only contains headings with a text length of 5 or more.
    // We use .trim() to remove any leading/trailing whitespace before checking the length.
    const filteredOutline = outline.filter(item => item.text && item.text.trim().length >= 5);

    return (
      <div className="flex-1 overflow-y-auto px-4 py-4">
        {/* The 4 feature tiles are always preserved at the top */}
        ...
        {/* This section dynamically displays the outline or a placeholder */}
        <div className="mt-8">
          {isLoadingOutline ? (
          ...
          ) : filteredOutline.length > 0 ? ( // Check the length of the new filtered array
            // If an outline is successfully loaded, display it
            <div className="flex flex-col gap-1">
              <h3 className="text-md font-semibold mb-2 px-2 text-neutral-300">Document Outline</h3>
              {/* Map over the new filtered array to render the buttons */}
            ...
            // If no outline is loaded, or all items were filtered out, show the original placeholder
